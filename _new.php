...
if (isset($baseFilename) && $baseFilename != '') {
	...
	$contentFile = 'p_'.$baseFilename.'.php';
} elseif (isset($tb)) {
	$phpFile = $tb.$phpExtension;
	//$contentFile = $tb.'Content.inc';
	$contentFile = 'p_'.$tb.'.php';
} else {
...
//		<td><i>None</i></td><td><i>No id field required</i></td></tr>
	...
	$sql = "insert into pages (page_name, page_ref) values ('$tb','p_$tb.php')";
...
\$_SESSION['current_page'] = 'p_$tb.php';
...
//CSV export data
\$_SESSION['csv_query'] = 'select * from $tb';
